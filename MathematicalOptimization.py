...
def c_from_b(b_m: float, xi: float, a: float) -> float:
...
def CALC_PARAMS(a,P_E_MW, B_max=13.0):
    # B_max = 13.0               # T ()
...
def objective_function(x):
    ...
    c,R0,A_p,V_p,p_atm,B0,beta_val,n,I_PM,epsilon,P_f,fB,qstar,beta_T,nG = \
        CALC_PARAMS(a, P_E_MW, B_max)
    ...
    # Bootstrap fraction
    fB = max(0.0, 0.8 - fB)**2
    ftot = wq*fq + wbeta*fbeta + wnG*fnG + wfB*fB
    return ftot
...
res = minimize(
    objective_function,
    x0=[2.0, 1000.0, 13.0], #Sets initial guess for a, P_E, B_max
...
if __name__ == "__main__":
    ...
    safety_limit(R0,B0,a,I_PM_H,kappa)
    beta_limit(beta_val,B0,I_PM_H,a)
    Density_limit(n,a,I_PM_H)
    bootstrap_frac(kappa,beta_val,B0,I_PM_H,R0,a,epsilon)
